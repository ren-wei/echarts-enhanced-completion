...
```
其中第一个参数 `value` 为 [data](~series-funnel.data) 中的数据值。第二个参数`params` 是其它的数据项参数。
...
1. 通过 [x](~series-funnel.markPoint.data.x), [y](~series-funnel.markPoint.data.y) 属性指定相对容器的屏幕坐标，单位像素，支持百分比。
...
标线两端的标记类型，可以是一个数组分别指定两端，也可以是单个统一指定，具体格式见 [data.symbol](~series-funnel.markLine.data.0.symbol)。
...
1. 通过 [x](~series-funnel.markLine.data.0.x), [y](~series-funnel.markLine.data.0.y) 属性指定相对容器的屏幕坐标，单位像素，支持百分比。
...
1. 通过 [x](~series-funnel.markArea.data.0.x), [y](~series-funnel.markArea.data.0.y) 属性指定相对容器的屏幕坐标，单位像素，支持百分比。
