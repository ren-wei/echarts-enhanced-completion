...
两端的文本，如['High', 'Low']。[参见例子](doc-example/map-visualMap-piecewise-text&edit=1&reset=1)。
...
`inRange` 能定义目标系列（参见 [visualMap-piecewise.seriesIndex](~visualMap-piecewise.seriesIndex)）视觉形式，也同时定义了 `visualMap-piecewise` 本身的视觉样式。通俗来讲就是，假如 `visualMap-piecewise`控制的是散点图，那么 `inRange` 同时定义了散点图的 `颜色`、`尺寸` 等，也定义了 `visualMap-piecewise` 本身的 `颜色`、`尺寸` 等。这二者能对应上。
...
配置参考 [visualMap-piecewise.inRange](~visualMap-piecewise.inRange)
...
这个配置项，是为了兼容 ECharts2 而存在，ECharts3 中已经不推荐使用。它的功能已经移到了 [visualMap-piecewise.inRange](~visualMap-piecewise.inRange) 和 [visualMap-piecewise.outOfRange](~visualMap-piecewise.outOfRange) 中。

如果要使用，则须注意，`color`属性中的顺序是由数值 `大` 到 `小`，但是 [visualMap-piecewise.inRange](~visualMap-piecewise.inRange) 或 [visualMap-piecewise.outOfRange](~visualMap-piecewise.outOfRange) 中 `color` 的顺序，总是由数值 `小` 到 `大`。二者不一致。
