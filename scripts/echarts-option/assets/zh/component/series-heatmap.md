...
+ `'cartesian2d'`
    ...
    使用二维的直角坐标系（也称笛卡尔坐标系），通过 [xAxisIndex](~series-heatmap.xAxisIndex), [yAxisIndex](~series-heatmap.yAxisIndex)指定相应的坐标轴组件。
...
+ `'geo'`
    ...
    使用地理坐标系，通过 [geoIndex](~series-heatmap.geoIndex) 指定相应的地理坐标系组件。
...
```
其中第一个参数 `value` 为 [data](~series-heatmap.data) 中的数据值。第二个参数`params` 是其它的数据项参数。
...
1. 通过 [x](~series-heatmap.markPoint.data.x), [y](~series-heatmap.markPoint.data.y) 属性指定相对容器的屏幕坐标，单位像素，支持百分比。
...
标线两端的标记类型，可以是一个数组分别指定两端，也可以是单个统一指定，具体格式见 [data.symbol](~series-heatmap.markLine.data.0.symbol)。
...
1. 通过 [x](~series-heatmap.markLine.data.0.x), [y](~series-heatmap.markLine.data.0.y) 属性指定相对容器的屏幕坐标，单位像素，支持百分比。
...
1. 通过 [x](~series-heatmap.markArea.data.0.x), [y](~series-heatmap.markArea.data.0.y) 属性指定相对容器的屏幕坐标，单位像素，支持百分比。
