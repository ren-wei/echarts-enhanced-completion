...
+ `'cartesian2d'`
    ...
    使用二维的直角坐标系（也称笛卡尔坐标系），通过 [xAxisIndex](~series-graph.xAxisIndex), [yAxisIndex](~series-graph.yAxisIndex)指定相应的坐标轴组件。



+ `'polar'`

    使用极坐标系，通过 [polarIndex](~series-graph.polarIndex) 指定相应的极坐标组件



+ `'geo'`

    使用地理坐标系，通过 [geoIndex](~series-graph.geoIndex) 指定相应的地理坐标系组件。
...
+ `'circular'` 采用环形布局，见示例 [Les Miserables](graph-circular-layout)，布局相关的配置项见 [graph.circular](~series-graph.circular)

+ `'force'` 采用力引导布局，见示例 [Force](graph-force)，布局相关的配置项见 [graph.force](~series-graph.force)
...
```
其中第一个参数 `value` 为 [data](~series-graph.data) 中的数据值。第二个参数`params` 是其它的数据项参数。
...
1. 通过 [x](~series-graph.markPoint.data.x), [y](~series-graph.markPoint.data.y) 属性指定相对容器的屏幕坐标，单位像素，支持百分比。


2. 用 [coord](~series-graph.markPoint.data.coord) 属性指定数据在相应坐标系上的坐标位置，单个维度支持设置 `'min'`, `'max'`, `'average'`。

3. 直接用 [type](~series-graph.markPoint.data.type) 属性标注系列中的最大值，最小值。这时候可以使用 [valueIndex](~series-graph.markPoint.data.valueIndex) 或者 [valueDim](~series-graph.markPoint.data.valueDim) 指定是在哪个维度上的最大值、最小值、平均值。
...
在使用 [type](~series-graph.markPoint.data.type) 时有效，用于指定在哪个维度上指定最大值最小值，可以是 `0`（xAxis, radiusAxis），`1`（yAxis, angleAxis），默认使用第一个数值轴所在的维度。

#### valueDim(string)

在使用 [type](~series-graph.markPoint.data.type) 时有效，用于指定在哪个维度上指定最大值最小值。这可以是维度的直接名称，例如折线图时可以是`x`、`angle`等、candlestick 图时可以是`open`、`close`等维度名称。
...
标线两端的标记类型，可以是一个数组分别指定两端，也可以是单个统一指定，具体格式见 [data.symbol](~series-graph.markLine.data.0.symbol)。
...
1. 通过 [x](~series-graph.markLine.data.0.x), [y](~series-graph.markLine.data.0.y) 属性指定相对容器的屏幕坐标，单位像素，支持百分比。


2. 用 [coord](~series-graph.markLine.data.0.coord) 属性指定数据在相应坐标系上的坐标位置，单个维度支持设置 `'min'`, `'max'`, `'average'`。

3. 直接用 [type](~series-graph.markLine.data.0.type) 属性标注系列中的最大值，最小值。这时候可以使用 [valueIndex](~series-graph.markLine.data.0.valueIndex) 或者 [valueDim](~series-graph.markLine.data.0.valueDim) 指定是在哪个维度上的最大值、最小值、平均值。
...
在使用 [type](~series-graph.markLine.data.type) 时有效，用于指定在哪个维度上指定最大值最小值，可以是 `0`（xAxis, radiusAxis），`1`（yAxis, angleAxis），默认使用第一个数值轴所在的维度。

##### valueDim(string)

在使用 [type](~series-graph.markLine.data.type) 时有效，用于指定在哪个维度上指定最大值最小值。这可以是维度的直接名称，例如折线图时可以是`x`、`angle`等、candlestick 图时可以是`open`、`close`等维度名称。
...
1. 通过 [x](~series-graph.markArea.data.0.x), [y](~series-graph.markArea.data.0.y) 属性指定相对容器的屏幕坐标，单位像素，支持百分比。


2. 用 [coord](~series-graph.markArea.data.0.coord) 属性指定数据在相应坐标系上的坐标位置，单个维度支持设置 `'min'`, `'max'`, `'average'`。

3. 直接用 [type](~series-graph.markArea.data.0.type) 属性标注系列中的最大值，最小值。这时候可以使用 [valueIndex](~series-graph.markArea.data.0.valueIndex) 或者 [valueDim](~series-graph.markPoint.data.0.valueDim) 指定是在哪个维度上的最大值、最小值、平均值。
...
在使用 [type](~series-graph.markArea.data.type) 时有效，用于指定在哪个维度上指定最大值最小值，可以是 `0`（xAxis, radiusAxis），`1`（yAxis, angleAxis），默认使用第一个数值轴所在的维度。

##### valueDim(string)

在使用 [type](~series-graph.markArea.data.type) 时有效，用于指定在哪个维度上指定最大值最小值。这可以是维度的直接名称，例如折线图时可以是`x`、`angle`等、candlestick 图时可以是`open`、`close`等维度名称。
...
## width(string|number) = 自适应

<ExampleUIControlPercent default="50%"/>

组件的宽度。

## height(string|number) = 自适应
