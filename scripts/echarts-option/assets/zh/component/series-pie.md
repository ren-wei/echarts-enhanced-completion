...
~[900x250](https://echarts.apache.org/examples/zh/view.html?c=pie-alignTo&reset=1&edit=1)
...
如果不需要开启该策略，例如[圆环图](pie-doughnut)这个例子中需要强制所有标签放在中心位置，可以将该值设为 `false`。
...
~[900x250](https://echarts.apache.org/examples/zh/view.html?c=doc-example/pie-label-margin&edit=1&reset=1)
...
~[800x250](https://echarts.apache.org/examples/zh/view.html?c=doc-example/pie-label-bleedMargin&edit=1&reset=1)
...
~[800x250](https://echarts.apache.org/examples/zh/view.html?c=doc-example/pie-label-distanceToLabelLine&edit=1&reset=1)
...
## radius(number|string|Array) = [0, \'75%\']
...
```
其中第一个参数 `value` 为 [data](~series-pie.data) 中的数据值。第二个参数`params` 是其它的数据项参数。
...
1. 通过 [x](~series-pie.markPoint.data.x), [y](~series-pie.markPoint.data.y) 属性指定相对容器的屏幕坐标，单位像素，支持百分比。
...
标线两端的标记类型，可以是一个数组分别指定两端，也可以是单个统一指定，具体格式见 [data.symbol](~series-pie.markLine.data.0.symbol)。
...
1. 通过 [x](~series-pie.markLine.data.0.x), [y](~series-pie.markLine.data.0.y) 属性指定相对容器的屏幕坐标，单位像素，支持百分比。
...
1. 通过 [x](~series-pie.markArea.data.0.x), [y](~series-pie.markArea.data.0.y) 属性指定相对容器的屏幕坐标，单位像素，支持百分比。
